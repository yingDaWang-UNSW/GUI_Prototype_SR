...
def argumentParser():
    ...
    parser.add_argument('Use_3D_Patches', metavar='Use 3D Patches', type=str, choices=['yes','no'], help='Use Patch based super resolution for 3D images (recommended)')
    
    parser.add_argument('Output_Format', metavar='Output Format', type=str, choices=['same','.png', '.jpg', '.mat', '.nc', '.tif'])
...
#    parser.add_argument('Bit_Depth', metavar='Bit Depth', type=str, choices=['uint8'], help='Bit depth of your input images  (Future releases will support more options)', default = 'uint8')
    ...
    parser.add_argument('Model_Type', metavar='Model Type', type=str, choices=['2D Rock', '2D Augmented Rock','3D Rock','2D Photo'], help='Dimensions of input images')
    ...
    parser.add_argument('Use_GAN', metavar='Use GAN', type=str, choices=['yes', 'no'], help='Force the network to use the GAN (will default to CNN)', default='no')
    
    #parser.add_argument('Use_PhotoSR', metavar='Use Photo-SR', type=str, choices=['yes', 'no'], help='Network will load checkpoints trained on 2D photographs', default='no')
...
@Gooey(program_name='Material Analysis by Machine Learning', default_size=(700, 700), image_dir="./image")
...
#                    stdout.write("\n")
                    ...
                    for i in range(lr.shape[3]*Scale_Factor):
                        print("\rSuper Resolving Orthogonal Slice %d" % (i+1))
                        lrSlice=temp2[:,:,:,i,:];
                        lrSlice=np.squeeze(lrSlice);
                        lrSlice=np.expand_dims(lrSlice,2)
                        lrSlice=np.expand_dims(np.concatenate((lrSlice, lrSlice, lrSlice), 2),0)
...
#                    stdout.write("\n")
                    ...
                    patchSize = 16
                    print(f'Super resolution will be applied to patches of size {patchSize}')
                    numsplitsX = lr.shape[1]//patchSize#16
                    numsplitsY = lr.shape[2]//patchSize#16
                    numsplitsZ = lr.shape[3]//patchSize#16
                    
                    lr=lr[:,0:patchSize*numsplitsX,0:patchSize*numsplitsY,0:patchSize*numsplitsZ,:]
...
#                #pdb.set_trace()
#                if daoutputFlagtasetBitDepth == 'uint8':
#                    lr = Image.open(path)
#                    if lr.mode != 'RGB':
#                        lr = lr.convert('RGB')
